...
$sql = "SELECT COUNT(*) AS total FROM mantis_bug_history_table
         WHERE field_name='" . $field_name . "' AND new_value='" . $prio_string
         . "' OR field_name='" . $field_name . "' AND new_value='" . $no_prio_string
         . "' OR field_name='" . $field_name . "' AND new_value='" . $no_change_string . "'";
$result = $conn->query($sql);
...
$result_2 = $conn->query($sql);
    if ($result_2->num_rows > 0) {
        while($row = $result_2->fetch_assoc()){
            ...
            $sql = "SELECT COUNT(*) AS total FROM mantis_bug_history_table WHERE id!="
            ...
            if($user_voted>0){
                ...
                if( $row["new_value"]=== $prio_string ){
                    if( intval($old_v)>intval($new_v) ){
                        $new_val=1;
                        deleteOldEntries( $conn,$history_id,$field_name,$user_id,$bug_id,0);
                    }
                ...
                $result->close();
                $z = " old_v: ".$old_v." new_v: ".$new_v. " new_val: " .$new_val;
            ...
            deleteOldEntries( $conn,$history_id,$field_name,$user_id,$bug_id,1);

            $old_val = $row["old_value"];
            $old_unchecked = $old_val;
            ...
            $info = "\n old_unchecked:" . $old_unchecked . "old: " .$old_val." new: ".$new_val." sum: "
            .$sum . " history_id: " .$history_id
            . " bug_id: " . $row["bug_id"] . " user_voted: " .$user_voted;
            
	    $set_value = "" . (1000 +intval($sum));
	    $set_value = substr($set_value, -3);

	    $sql = "UPDATE mantis_custom_field_string_table SET value ='".$set_value . "' WHERE field_id = " . $field_id ." AND bug_id = " . $bug_id;

            $conn->query($sql);
            $sql = "UPDATE mantis_bug_history_table SET new_value ='".$set_value . "' WHERE id = " . $history_id . " AND field_name = '" .$field_name."' AND bug_id = " . $bug_id;
            $conn->query($sql);
            $file = 'vote.log';
            $line =  "\n" . $dat . " " . $sql . " INFO:" . $info ."\nz: " . $z;
...
function deleteOldEntries( $conn, $history_id, $field_name, $user_id, $bug_id, $choice ){
